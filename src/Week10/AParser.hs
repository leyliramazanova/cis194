...
Hint: You may find it useful to implement a function first :: (a -> b) -> (a,c) -> (b,c)
-}
...
• p1 <*> p2 represents the parser which first runs p1 (which will consume some input and produce a function), 
then passes the remaining input to p2 (which consumes more input and produces some value), 
then returns the result of applying the function to the value. 
However, if either p1 or p2 fails then the whole thing should also fail (put another way, p1 <*> p2 only succeeds if both p1 and p2 succeed).
So what is this good for? Recalling the Employee example from class,
type Name = String
 data Employee = Emp { name :: Name, phone :: String }
we could now use the Applicative instance for Parser to make an employee parser from name and phone parsers. 
That is, if
 parseName  :: Parser Name
 parsePhone :: Parser String
then
 Emp <$> parseName <*> parsePhone :: Parser Employee
is a parser which first reads a name from the input, then a phone number, and returns them combined into an Employee record. 
Of course, this assumes that the name and phone number are right next to each other in the input, with no intervening separators. 
...
You should implement each of the following exercises using the Applicative interface to put together simpler parsers into more complex ones. 
Do not implement them using the low-level definition of a Parser! In other words, pre- tend that you do not have access to the Parser constructor or even know how the Parser type is defined.
• Create a parser
...
*AParser> runParser abParser_ "abcdef" Just ((),"cdef")
*AParser> runParser abParser_ "aebcdf" Nothing
...
formats. But for any format involving choice (e.g. “. . . after the colon there can be a number or a word or parentheses. . . ”) 
Applicative is not quite enough. To handle choice we turn to the Alternative class, defined (essentially) as follows:
